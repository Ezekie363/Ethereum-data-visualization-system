...
1. 先展示 30 天窗口默认页面
2. 切换到 7 天和 90 天，说明窗口对指标的影响
3. 展示 Top10 巨鲸表与资金流向饼图
4. 展示“指标方法说明”证明口径可追溯
...
- `daily_metrics.csv` 行数应等于窗口天数
- 日期应连续且升序
- `whale_ranking.csv` 中 `netflow_eth ≈ inflow_eth - outflow_eth`
- 饼图占比总和应约等于 100%
...
- CSV 缺失时，前端可回退到 mock 数据并提示来源变化（用于防止页面空白）

---

## 11. 已知边界与学术说明

1. 本系统优先离线复现，不追求逐笔交易级精确还原。
2. 巨鲸与分布为公开统计驱动的代理分析，不是地址级全量链数据回放。
3. 若论文要求“逐笔真实转账金额分布”，需后续引入更细粒度数据仓库（如 Dune 查询结果或自建 ETL）。

在论文中建议明确写为：

- “本研究采用公开日度统计与账户快照构建离线分析体系，重点验证系统架构、指标方法与可视化表达，不以逐笔事件级重建为目标。”
...
- 浏览器强制刷新：`Cmd + Shift + R`
- 重新生成离线数据：`./refresh_offline_data.sh`

### Q3: 某个窗口加载失败

- 检查 `dashboard/data/ethereum/{7|30|90}/` 下四个文件是否齐全
- 检查是否在项目根目录启动了 HTTP 服务（不能直接双击 html）
...
1. 增量更新机制（仅更新最近 1 天）
2. 增加链上指标（稳定币净流入、合约交互占比、MEV 代理）
3. 引入可追溯数据仓（SQLite/Parquet）
4. 导出论文图版（PNG/SVG 批量导出）
